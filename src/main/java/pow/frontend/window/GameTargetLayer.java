...
public class GameTargetLayer extends AbstractWindow {
    ...
    public void drawContents(Graphics graphics) {
        ...
        if (mode == TargetMode.TARGET) {
            ...
            GameState gs = backend.getGameState();
            int radius = gs.party.player.viewRadius;
            if (!cursorPosition.equals(gs.party.player.loc)) {
                List<Point> ray = Bresenham.makeRay(gs.party.player.loc, cursorPosition, radius + 1);
                for (Point p : ray) {
                    if (!gs.party.player.canSeeLocation(gs, p)) break;
    ...
    private void update() {
        ...
        if (selectedActor != null) {
            if (!gs.party.player.canSeeLocation(gs, selectedActor.loc) || !gs.party.player.canSeeActor(selectedActor)) {
                selectedActor = null;
            }
        }
    ...
    private String makeMessage() {
        ...
        if (gs.party.player.canSeeLocation(gs, cursorPosition)) {
            StringBuilder sb = new StringBuilder();
            sb.append("you see ");

            List<String> interestingThings = new ArrayList<>();
            Actor actor = gs.getCurrentMap().actorAt(x,y);
            if (actor != null && gs.party.player.canSeeActor(actor)) {
